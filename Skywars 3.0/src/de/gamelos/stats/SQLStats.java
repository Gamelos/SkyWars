...
public class SQLStats {
	...
	public static boolean playerExists(String uuid){
		...
		try {
			...
			ResultSet rs = de.gamelos.skywars.Main.mysql.querry("SELECT * FROM Skywars WHERE UUID = '"+ uuid + "'");
	...
	public static void createPlayer(String uuid, String name){
		...
		if(!(playerExists(uuid))){
				name1 = name;
			de.gamelos.skywars.Main.mysql.update("INSERT INTO Skywars(UUID, KILLS, TODE, WINS, GAMES, NAME, COINS) VALUES ('" +uuid+ "', '0', '0', '0', '0', '"+name1+"', '0');");
		}
	}
	...
	public static void setKills(String uuid, Integer kills, String name){
		...
		if(playerExists(uuid)){
			de.gamelos.skywars.Main.mysql.update("UPDATE Skywars SET KILLS= '" + kills+ "' WHERE UUID= '" + uuid+ "';");
		}else{
			createPlayer(uuid, name);
	...
	public static void setTode(String uuid, Integer tode, String name){
		...
		if(playerExists(uuid)){
			de.gamelos.skywars.Main.mysql.update("UPDATE Skywars SET TODE= '" + tode+ "' WHERE UUID= '" + uuid+ "';");
		}else{
			createPlayer(uuid, name);
	...
	public static void setWins(String uuid, Integer tode, String name){
		...
		if(playerExists(uuid)){
			de.gamelos.skywars.Main.mysql.update("UPDATE Skywars SET WINS= '" + tode+ "' WHERE UUID= '" + uuid+ "';");
		}else{
			createPlayer(uuid, name);
	...
	public static void setGames(String uuid, Integer tode, String name){
		...
		if(playerExists(uuid)){
			de.gamelos.skywars.Main.mysql.update("UPDATE Skywars SET GAMES= '" + tode+ "' WHERE UUID= '" + uuid+ "';");
		}else{
			createPlayer(uuid, name);
	...
	public static void setCoins(String uuid, Integer tode, String name){
		...
		if(playerExists(uuid)){
			de.gamelos.skywars.Main.mysql.update("UPDATE Skywars SET COINS= '" + tode+ "' WHERE UUID= '" + uuid+ "';");
		}else{
			createPlayer(uuid, name);
	...
	public static void setName(String uuid, String name){
		...
		if(playerExists(uuid)){
			de.gamelos.skywars.Main.mysql.update("UPDATE Skywars SET NAME= '" + name+ "' WHERE UUID= '" + uuid+ "';");
		}
	}
...
//	======================================================================================
	public static Integer getUserRanking(String uuid){
		@SuppressWarnings("static-access")
		ResultSet rs = de.gamelos.skywars.Main.mysql.querry("SELECT * FROM Skywars ORDER BY WINS DESC");
		int count = 0;
		try {
			while(rs.next()){
				count++;
